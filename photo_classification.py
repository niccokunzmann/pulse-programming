...
colored_ness = colored_ness.reshape(-1, 1)
#colored_ness = np.array([[how_colored_is_this_normalized(c) for c in row] for row in image])
gmm = GaussianMixture(n_components=2).fit(colored_ness)
gate_image = np.array(gmm.predict(colored_ness).reshape(DIMENSIONS), np.uint8)
gate_image *= 255
